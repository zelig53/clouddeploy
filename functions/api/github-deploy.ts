...
export const onRequestPost: PagesFunction = async ({ request }) => {
  try {
    ...
    const GH = (url: string, opts: RequestInit = {}) =>
      fetch(url, { ...opts, headers: { Authorization: `Bearer ${githubToken}`, "User-Agent": "CloudDeploy-Mobile", Accept: "application/json", "Content-Type": "application/json", ...(opts.headers || {}) } });
    ...
    if (repoCheck.status === 404) {
      const cr = await GH("https://api.github.com/user/repos", { method: "POST", body: JSON.stringify({ name: repoName, auto_init: true }) });
    ...
    const zip = new JSZip();
    const content = await zip.loadAsync(await zipBytes);
    const fileNames = Object.keys(content.files).filter(n => !content.files[n].dir && !n.includes("__MACOSX") && !n.includes(".DS_Store"));
    ...
    if (branch !== "main" && targetSha) {
      ...
      if (branchCheck.status === 404) {
        await GH(`https://api.github.com/repos/${username}/${repoName}/git/refs`, { method: "POST", body: JSON.stringify({ ref: `refs/heads/${branch}`, sha: targetSha }) });
      } else {
    ...
    for (const filename of fileNames) {
      ...
      const blobRes = await GH(`https://api.github.com/repos/${username}/${repoName}/git/blobs`, { method: "POST", body: JSON.stringify({ content: b64, encoding: "base64" }) });
    ...
    // Create tree
    const treeRes = await GH(`https://api.github.com/repos/${username}/${repoName}/git/trees`, { method: "POST", body: JSON.stringify({ base_tree: targetSha, tree: treeItems }) });
    ...
    // Create commit
    const commitRes = await GH(`https://api.github.com/repos/${username}/${repoName}/git/commits`, { method: "POST", body: JSON.stringify({ message: commitMessage || "Deploy from CloudDeploy", tree: treeSha, parents: targetSha ? [targetSha] : [] }) });
    ...
    // Update ref
    const refRes = await GH(`https://api.github.com/repos/${username}/${repoName}/git/refs/heads/${branch}`, { method: "PATCH", body: JSON.stringify({ sha: commitSha, force: true }) });
    if (!refRes.ok) { const e = await refRes.json() as any; throw new Error(`Ref update failed: ${e.message}`); }

    // PR if not main
    let prUrl = null;
    if (branch !== "main") {
      const prRes = await GH(`https://api.github.com/repos/${username}/${repoName}/pulls`, { method: "POST", body: JSON.stringify({ title: `Deploy: ${commitMessage || "New update"}`, head: branch, base: "main", body: "Created by CloudDeploy" }) });
      if (prRes.ok) { const pd = await prRes.json() as any; prUrl = pd.html_url; }
    }

    return Response.json({ success: true, repoUrl: `https://github.com/${username}/${repoName}`, prUrl, cloudflareUrl: `https://${projectName || repoName}.pages.dev`, branch });
  } catch (e: any) {
